...
void color_dominante(int clear, int red, int green, int blue) {
  ...
  sprintf(message, "Color sensor: clear: %d ,Red: %d ,Green:%d blue:%d--",
  ...
  sprintf(message,
          "Color sensor: clear: %d, Red: %d, Green: %d, Blue: %d -- Dominant "
          "color: %s\n",
...
void F_Soil(uint16_t valor) {
  float moisture =
      ((float)valor / 65535) * 100; // o puede ser 100 -(valor /65535)* 100;
...
void F_calculo() {
  ...
  sprintf(buffer,
          " Total values %d,  Temperature : Array1 ,humidity: Array2 , Ambient "
          "Light : Array 3 , Soid moisture : Array4 \n",
          cont_h);
  pc.write(buffer, strlen(buffer));

  for (int i = 0; i < 4; i++) {
    sprintf(buffer, "Array %d - Promedio: %f, Máximo: %f, Mínimo: %f\n", i + 1,
            MyData.average[i], MyData.maximum[i], MyData.minimun[i]);
    pc.write(buffer, strlen(buffer));
  }

  volatile float max_x = MyData.accel[0][0], max_y = MyData.accel[1][0],
                 max_z = MyData.accel[2][0], min_x = MyData.accel[0][0],
                 min_y = MyData.accel[1][0], min_z = MyData.accel[2][0];
...
void F_color_Dominant() {
  ...
  if (MyData.dominant_color[0] >= MyData.dominant_color[1] &&
      MyData.dominant_color[0] >= MyData.dominant_color[2]) {
    dominant_color = "red";
  } else if (MyData.dominant_color[1] >= MyData.dominant_color[0] &&
             MyData.dominant_color[1] >= MyData.dominant_color[2]) {
    dominant_color = "green";
  } else {
    dominant_color = "blue";
  }
  ...
  sprintf(message,
          "number of times it appears as a dominant color: red %d , green %d , "
          "blue %d . the dominant color with the most times is %s \n",
          MyData.dominant_color[0], MyData.dominant_color[1],
          MyData.dominant_color[2], dominant_color.c_str());
  pc.write(message, strlen(message));

  MyData.dominant_color[0] = 0;
  MyData.dominant_color[1] = 0;
  MyData.dominant_color[2] = 0;
}
...
int main() {
  ...
  while (true) {
    ...
    case NORMAL:
      ...
      if (humid_meas.temp > 30 || humid_meas.temp < -10) {
        Led_red = 0;
        Led_blue = 1;
        Led_green = 1;
        ...
        Led_blue = 0;
        Led_green = 1;
      } else if (color_data.r > 0.5 * color_data.c ||
                 color_data.b > 0.5 * color_data.c) {
        Led_red = 1;
        Led_blue = 0;
        Led_green = 0;
      } else if ((acceleration.x > 0.5 || acceleration.y > 0.5) &&
                 acceleration.z < 0.7) {
        Led_red = 0;
        Led_blue = 0;
        Led_green = 0;
